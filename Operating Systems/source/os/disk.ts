module TSOS {
    export class Disk extends DeviceDriver {
        private deviceName: string;
        private tracks: number;
        private sectors: number;
        private blocks: number;
        private blockSize: number = 64;
        public hexBlockSize: number = this.blockSize * 2;

        private directory: number = 0;  // Track 0 = directory
        private freeBlockMap: boolean[][][];
            ...
            for (let t = 0; t < this.tracks; t++) {
                for (let s = 0; s < this.sectors; s++) {
                    for (let b = 0; b < this.blocks; b++) {
                        ...
                        if (oldBlock === null) oldBlock = "".padEnd(this.hexBlockSize, "0");
        ...
        private key(t: number, s: number, b: number): string {
            return `${this.deviceName}_${t}${s}${b}`;
        }

        public getBlock(t: number, s: number, b: number): string {
            const k = this.key(t, s, b);
            const content = sessionStorage.getItem(k);
            if (content === null) throw new Error(`Missing disk block: ${k}`);
            return content;
        }

        public setBlock(t: number, s: number, b: number, data: string): void {
        ...
        public allocateDBlock(): [number, number, number] {
            for (let t = 0; t < this.tracks; t++) {
                for (let s = 0; s < this.sectors; s++) {
                    for (let b = 0; b < this.blocks; b++) {
                        // Skip reserved directory block
                        if (t === 0 && s === 0 && b === 0) continue;
        ...
        public freeBlock(t: number, s: number, b: number): void {
            this.freeBlockMap[t][s][b] = true;
        }

        public createFile(filename: string): boolean {
            const dir = this.findDirectory(filename);
            if (dir) return false; // file exists
        ...
        public listFilesAll(): { name: string, size: number, date: number, hidden: boolean }[] {
            const files = [];
            ...
            for (let s = 0; s < this.sectors; s++) {
                for (let b = 0; b < this.blocks; b++) {
                    ...
                    if (!block.startsWith("01")) continue;

                    const dt = block.substring(2, 8);
                    if (dt == "000000") continue;
        ...
        private findDirectory(filename: string): [number, number, number] | null {
            const hexName = this.stringToHex(filename);
        ...
        private stringToHex(s: string): string {
            let out = "";
            for (let c of s) out += c.charCodeAt(0).toString(16).padStart(2, "0");
            return out;
        }
        ...
        public readSwap(t: number, s: number, b: number): { bytes: string[], chain: [number, number, number][] } {
        ...
        public writeSwap(pid: number, data: string[]): [number, number, number] {
            ...
            const tsbs: [number, number, number][] = [];
            try {
